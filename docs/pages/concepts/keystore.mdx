...
Base and Celo host a cache:

- **Base** is a verification target. Sessions are granted on Ethereum, and [`ensureKeyCached`](/sdk/sync-to-l2) mirrors them to Base for readers there. Base has no relay.
- **Celo** is an execution network. Wallets run through the Altana relay on Celo, their sessions are recorded in the Ethereum Keystore, and `grantSession` proves each entry into the cache on Celo; `revokeSession` proves the revocation. `registryNetwork(CELO)` returns `ETHEREUM`, the chain the SDK reads. See [Celo](/sdk/celo).
...
| Celo | 42220 | Wallet execution; Keystore on Ethereum, L2 Keystore cache on Celo |
...
For testnet, the SDK also exports `BNB_TESTNET` (chain 97), `CELO_SEPOLIA` (chain 11142220) and `SEPOLIA` (chain 11155111, the Keystore behind Celo Sepolia). See [Testnet](/concepts/networks/testnet).
