...
So the SDK has two shapes of network:

- **L1** (BNB Smart Chain, Ethereum): the Keystore is on the chain itself. A grant is one relay transaction that registers the key and authorizes the account.
- **L2** (Base, Celo): the Keystore is on the L1 (Ethereum; Sepolia on testnet) and a KeyStoreCache on the L2 mirrors it. `grantSession` writes the L1 Keystore, authorizes the account through the L2's relay, then proves the entry into the L2 cache; `revokeSession` revokes the account, the L1 entry, then proves the revocation. `registryNetwork(network)` returns the L1 the SDK reads for any config. [`ensureKeyCached`](/sdk/sync-to-l2) mirrors an Ethereum-granted session to Base for readers there.

The cache's `isValidKey` answers while the entry was proven at the L1 block the L2 currently anchors (the SDK's `isCachedKeyValid` maps its revert to `false`); `getCachedKey` returns the proven entry.
...
| Celo | 42220 | L2: wallet execution, Keystore on Ethereum, Keystore cache on Celo |

Mainnet exports: `BNB`, `ETHEREUM`, `BASE`, and `CELO`. BNB, Ethereum and Celo support wallet writes. Base is the L2 cache/read target for verifying Ethereum-granted sessions cross-chain.

For testnet, the SDK also exports `BNB_TESTNET` (chain 97), `CELO_SEPOLIA` (chain 11142220, L2) and `SEPOLIA` (chain 11155111, its L1 Keystore). See [Testnet](/concepts/networks/testnet).
