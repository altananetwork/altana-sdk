...
`isValidKey` folds the three questions that matter into one read: the key exists, it has not been revoked, and it has not expired. To list every key on a wallet rather than check one, read `getKeys(user)`, which returns `bytes32[]`. It enumerates registered key ids only and does **not** filter out revoked or expired ones, so pair it with `isValidKey` per key when liveness matters.
