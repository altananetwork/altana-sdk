...
:::info[L2 execution]
The SDK supports wallet execution on BNB, Ethereum and Celo. Cross-chain **verification** from Base is available via `ensureKeyCached` + `isValidKey`; on Celo, `grantSession` proves the session into the Celo cache itself (see [Celo](/sdk/celo)).
:::
