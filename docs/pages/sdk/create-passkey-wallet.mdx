...
**Browser by default** (uses `navigator.credentials`) — **or any JavaScript runtime that supplies WebAuthn functions**: in React Native, Expo, or Capacitor, pass your native passkey library's functions via the `webAuthn` option and users get the platform's real Face ID / fingerprint passkeys — see [Use the SDK in a mobile app](/use-cases/8-mobile-app).
...
The chains the wallet is provisioned on come from the client (`createClient({ chains })`). For wallet execution, use `BNB`, `ETHEREUM` or `CELO`; `BASE` is exported for the L2 Keystore cache used by cross-chain verification.
