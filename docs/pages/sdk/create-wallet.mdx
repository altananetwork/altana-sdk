...
- Fund `result.address` with native tokens before calling `execute` — the wallet is counterfactual and has no balance until you send some. The first transaction pays the KeyStore registration fee and the relay fee from that balance; unfunded, it throws `The relay rejected the request to prepare the call: 0x (empty revert: wallet … holds no native balance …)` (see [Errors](/sdk/errors)).
- **Browser wallets (MetaMask, Trust Wallet, Rabby) cannot be the signer** — they withhold the signatures a 7702 account needs. They make excellent *funding sources* instead: see [Onboard users from browser wallets](/use-cases/7-onboard-from-browser-wallets) for the connect → passkey → one-click-funding flow.
