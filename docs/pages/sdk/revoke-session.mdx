...
1. The session is **revoked in Keystore** (skipped if the session was never registered). Any tool reading `isValidKey` for that key will get `false` from the next block onward.
