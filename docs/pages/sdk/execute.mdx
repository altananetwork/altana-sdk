...
- **The relay fee is paid in the native currency (BNB), not `$U`.** `feeToken` defaults to native and you rarely need to set it; if you do, it must be a token the relay accepts (native on testnet). `$U` is for ERC-8183 job escrow and x402 payments — not relay fees. Passing an unaccepted `feeToken` is rejected before the call runs, now with a clear "fee token not supported" message.
