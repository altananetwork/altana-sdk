...
| `wallet_execute` | Submits one or more calls signed by the wallet's admin key. The relay takes its fee in whichever accepted token the wallet holds; the result names the token charged. |
