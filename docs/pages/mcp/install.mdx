...
| `celo-sepolia` | Celo Sepolia (11142220); Keystore on Sepolia, cache on Celo Sepolia |
| `celo` | Celo (42220); Keystore on Ethereum, cache on Celo |
...
# Operate on Celo Sepolia (chain 11142220)
...
On `celo-sepolia` and `celo` the Keystore is on Sepolia and Ethereum respectively. `wallet_verification` and `verify_authorization` read it there and add a `cache` block with what the KeyStoreCache on Celo holds; `grant_session` and `revoke_session` report the Keystore write and the cache proof. See [Celo Sepolia](/sdk/celo-sepolia) and [Celo](/sdk/celo).
