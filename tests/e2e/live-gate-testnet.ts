...
async function main() {
  ...
  check("cached expiry matches the L1 expiry", Number(cached.expiry), session.expiry);
  const afterBridge = await l2.readContract({
    address: wallet.address,
    abi: CAN_EXECUTE_ABI,
    functionName: "canExecute",
    args: [keyHash, TARGET, "0x"],
  ...
  const afterRevoke = await l2.readContract({
    address: wallet.address,
    abi: CAN_EXECUTE_ABI,
    functionName: "canExecute",
    args: [keyHash, TARGET, "0x"],
