...
SEPOLIA_RPC_URL=https://.../<KEY>           # fork-celo-cache, smoke:celo-sepolia: optional override of SEPOLIA.publicRpcUrl
                                            # (must serve eth_getProof for the block Celo Sepolia anchors, about 100 behind head)
```
...
`smoke-celo-fee-currency.ts` proves relay fees paid in a stablecoin: it lists the relay's fee tokens (`client.feeCurrencies()`), then for each token under test (default `usdc,eurm`, override with `SMOKE_FEE_TOKENS`) creates a wallet, funds it with that token and no CELO, executes with no `feeToken`, and checks the result names the token, the fee came out of the token balance, and the CELO balance is still zero. It then grants a session with `feeTokens: [token]` (`register: false`) and executes as the session with nothing named, checking the session paid in the token from the cap the grant added. Leftover tokens are swept back to the funder through the relay, paying that fee in the token too. It needs `TEST_FUNDER_KEY` holding the tokens on Celo Sepolia (USDC: https://faucet.circle.com; USDm, EURm, KESm: https://faucet.celo.org/celo-sepolia) and fails loudly naming the missing one.

```bash
bun run --filter '@altananetwork/e2e' smoke:celo-fee-currency
```
